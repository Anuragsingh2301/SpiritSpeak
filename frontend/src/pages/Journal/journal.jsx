...
const Journal = () => { 
    ...
    const [activeGuide, setActiveGuide] = React.useState(guidesData[0]);
    const wordCount = journalText.split(/\s+/).filter(Boolean).length;

    const handleGetReflection = () => {
        const mockReflections = { kai: "It sounds like today held a mix of challenges and small victories...", elara: "Your words paint such a vivid picture of your day! It's like you're weaving a tapestry of experiences...", orion: "Analyzing the events of your day, a clear pattern emerges around your reactions...", aethel: "The feelings you describe today are like leaves on a branch; let's look at the root they grew from..." };
        const newReflection = { id: Date.now(), guide: activeGuide, text: mockReflections[activeGuide.id] };
                ...
                <div className="p-4 md:p-0 h-full flex flex-col gap-8">
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-lg flex-grow flex flex-col">
                        ...
                        <div className="flex flex-wrap items-center justify-end gap-4 mt-4">
                            <select onChange={(e) => setActiveGuide(guidesData.find(g => g.id === e.target.value))} className="bg-white border border-stone-300 rounded-lg p-2 font-semibold focus:ring-2 focus:ring-teal-500">
